...
def _handle_path(path, sess, samples_each_categpry, low_profile=False):
    if path.endswith('.npz'):
        ...
        new_files = files[:samples_each_categpry]
        if low_profile:
            m, s = calculate_activation_statistics_from_files(new_files, sess)
        else:
            x = np.array([imread(str(fn)).astype(np.float32) for fn in new_files])
            print('herer',np.shape(x))
            m, s = calculate_activation_statistics(x, sess)
            del x #clean up memory
    return m, s


def calculate_fid_given_paths_tensorflow(paths, samples_each_categpry, inception_path=None, low_profile=False):
    ...
    with tf.Session() as sess:
        ...
        m1, s1 = _handle_path(paths[0], sess,samples_each_categpry, low_profile=low_profile)
        m2, s2 = _handle_path(paths[1], sess,samples_each_categpry, low_profile=low_profile)
